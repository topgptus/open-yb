...
这一版把“微信生态内容交给元宝处理”的主流程补清楚。

- 新增公众号快捷按钮：打开 `mp.weixin.qq.com/s/...` 文章时，页面右上角显示“发给元宝”。
- 点击后复制：当前公众号文章 URL + 可配置的元宝提炼提示词。
- 新增元宝提示词配置：收藏库设置页可以开关公众号按钮，也可以编辑默认提示词。
- LLM 整理增加开关：默认关闭。关闭时不会显示“AI 整理后收藏”的右键菜单。
- 产品边界调整：公众号和视频号这类微信生态内容，优先交给元宝处理；本地 LLM 只作为选中文本整理的可选增强。

推荐用法：

```text
打开公众号文章 -> 点击“发给元宝” -> 粘贴到微信元宝 -> 元宝生成分享链接 -> Open YB 解析/收藏/导出
```

### v0.4.0

这一版新增右键摘录和本地 LLM 整理能力。

- 新增右键菜单：选中文本后可以直接“保存选中到 Open YB”。
- 新增 AI 右键菜单：选中文本后可以“AI 整理后收藏”，调用你配置的 OpenAI 兼容模型。
- 新增 LLM 设置：支持配置 `Base URL`、`API Key`、`Model` 和默认整理提示词。
- 新增来源筛选：收藏库可以区分“选中摘录”和“AI 摘录”。
- 标签策略更清晰：普通网页/选中摘录不自动扫 `#`，只有元宝分享和 AI 整理结果会提取 `#标签`。

推荐用法：

```text
网页里选中一段内容 -> 右键 -> AI 整理后收藏 -> 收藏库按 AI 摘录/tag/日期筛选 -> 导出 Markdown
```

### v0.3.0

这一版新增轻量网页剪藏能力，思路类似 MarkDownload：不依赖外部插件，点击后直接把当前网页转换成 Markdown 并保存到 Open YB 收藏库。

- 新增“保存当前网页为 MD”：在任意网页点击插件按钮即可剪藏当前页面。
- 支持选中文本优先：如果页面里有选中文本，会优先保存选中部分；否则提取正文区域。
- 支持基础 HTML 转 Markdown：标题、段落、链接、图片、列表、代码块、引用、表格会尽量转换成 Markdown。
- 收藏库新增来源筛选：可以按“元宝分享 / 网页剪藏”区分内容。
- 继续复用自动标签、日期筛选、批量导出和合并导出能力。

推荐用法：

```text
网页资料 -> 点击“保存当前网页为 MD” -> 自动进入收藏库 -> 按来源/tag/日期筛选 -> 导出 Markdown
```
...
如果是普通网页，不需要先转发给元宝。直接打开网页后点击插件里的“保存当前网页为 MD”即可。插件会优先保存当前选中文本；没有选中文本时，会尝试提取正文区域并转换成 Markdown。
...
- `保存当前网页为 MD`：在任意网页点击插件按钮，把当前网页或选中文本保存为 Markdown 收藏。
